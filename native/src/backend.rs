...
const BACKEND_NAME: &str = "handbrake-mcp-backend.exe";
const BACKEND_PORT: u16 = 10875;
...
fn dev_backend_path() -> Option<PathBuf> {
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("binaries")
        .join("handbrake-mcp-backend-x86_64-pc-windows-msvc.exe");
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) = app
        .path()
        .resolve("resources/handbrake-mcp-backend.exe", BaseDirectory::Resource)
    {
        tried.push(path.display().to_string());
        if path.exists() {
            return Ok(path);
...
fn free_port(port: u16) -> bool {
    ...
    {
        let img_kill = format!(
            "Stop-Process -Name 'handbrake-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'handbrake-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM handbrake-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM handbrake-mcp-native.exe /T 2>$null"
        );
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name handbrake-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM handbrake-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
