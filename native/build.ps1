...
$apiFile = "$Root\web_sota\src\lib\api.ts"
if (Test-Path $apiFile) {
    $apiContent = Get-Content $apiFile -Raw
    if ($apiContent -match "127.0.0.1:(\d+)") {
        $apiPort = [int]$Matches[1]
        if ($apiPort -ne 10875) {
            throw "API_BASE in $apiFile points to port $apiPort but backend serves on 10875. In dev Vite proxies work, in prod/NSIS this gives 'Failed to fetch'."
        }
