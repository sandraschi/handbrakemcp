...
from handbrake_mcp.services.handbrake import TranscodeJob, get_handbrake_service
from handbrake_mcp.services.notification_service import NotificationRecipient, NotificationService
...
class ProcessingService:
    ...
    def __init__(self):
        ...
        self._on_job_complete: Callable[[TranscodeJob], None] | None = None

        # Configure email notifications if settings are available
        if settings.smtp_server and settings.email_notifications and settings.email_recipients:
            ...
            for email in settings.email_recipients:
                self.notification_service.add_recipient(NotificationRecipient(email=email))
